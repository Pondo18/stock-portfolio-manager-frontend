...
class Model:
    ...
    def get_username_by_hashcode(self, hashcode):
        ...
        r = requests.get(f"{self.cfg['api_endpoint']}/users", params=payload)
        username = r.json()[0]["username"]
        return username

    def get_credits_by_username(self, username):
        r = requests.get(f"{self.cfg['api_endpoint']}/users/{username}")
    ...
    def create_new_holding(self, username, holding, number, buy_in):
        ...
        r = requests.post(f"{self.cfg['api_endpoint']}/users/{username}/holdings", json=entry_data)
        return r.json()

    def update_number_of_holdings(self, username, holding, number, buy_in):
        entry_data = {'number': number, 'buyIn': buy_in}
        r = requests.put(f"{self.cfg['api_endpoint']}/users/{username}/holdings/{holding}", json=entry_data)
        return r.json()

    def update_credits_in_database(self, username, new_credits):
        entry_data = {'credits': new_credits}
        r = requests.put(f"{self.cfg['api_endpoint']}/users/{username}", json=entry_data)
        return r.json()

    def get_amount_of_holdings_user_already_has(self, username, holding):
        r = requests.get(f"{self.cfg['api_endpoint']}/users/{username}/holdings/{holding}")
        try:
